...
try:
    ...
    print(f'Failed loading proxy config: {e}')
...
def browser_cmd(exe: Path, agent: str, profile: str = '') -> list[str]:
    ...
    if profile:
        ...
        # Apply SOCKS5 proxy per profile
        proxy = PROXY_MAP.get(profile)

        if proxy:
            print(f'Using proxy for "{profile}": {proxy}')
